...
- [ ] **Completar el XML SES.HOSPEDAJES conforme al esquema oficial** — el documento oficial del Ministerio del Interior ya está disponible y auditado (ver sección "Cumplimiento SES.HOSPEDAJES/RD 933/2021" arriba, 2026-07-15); el formulario y el modelo de datos ya se corrigieron, pero la **estructura del XML en sí** sigue sin ser conforme al 100%. Todo lo de abajo está bloqueado hasta resolver los dos primeros puntos:
  - [ ] **`codigoEstablecimiento`** — campo obligatorio en la cabecera (`String`, máximo 10 caracteres), lo asigna el Sistema de Hospedajes del Ministerio del Interior al dar de alta al titular. **Es un trámite distinto de la licencia turística `CV-VUT0058371-V`** que ya se tiene (esa es del Registro de Turismo de la Comunitat Valenciana, autonómica) — la licencia CV-VUT tiene 16 caracteres, no cabría en el campo aunque fuera el mismo dato. **Pendiente de que el usuario confirme si ya estáis dados de alta como titulares en ese Sistema de Hospedajes**; sin ese código el XML no lo va a aceptar el portal real
  - [ ] **`fechaContrato`** — obligatoria, no se emite todavía. Falta decidir qué fecha usar como tal (¿la de creación de la reserva? ¿la del pago del depósito?)
  - [ ] **Bloque `pago`** — obligatorio, no existe ni en el modelo de datos ni en el XML. Como mínimo hace falta declarar `tipoPago`; hay que decidir qué datos de pago (ya se tienen de Stripe en el modelo `Booking`) se usan para rellenarlo
  - [ ] **`rol` = "VI"** — obligatorio en cada viajero de un "parte de viajeros", no se emite en el XML actual (es un valor fijo, fácil una vez se aborde el resto)
  - [ ] **Reestructurar el envoltorio del XML** — hoy genera `<parteViajeros><establecimiento>...`, pero la especificación oficial exige `<peticion><solicitud><codigoEstablecimiento/><comunicacion><contrato>...<persona>...`. El bloque `<establecimiento>` actual (nombre/licencia/NIF/dirección) **no existe en absoluto** en la especificación de "Partes de viajeros" — es una invención del código original (aunque el propio fichero ya avisaba de que era un borrador no contrastado), hay que quitarlo cuando se aborde esta fase
