...
- [ ] Todos los hallazgos de la auditoría de calendario están resueltos.
...
2. **Añadir `ICAL_EXPORT_TOKEN` en Railway** (mismo valor que en `backend/.env` local, o uno distinto generado igual: `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`) — sin esta variable el backend no arrancará en producción (`requireEnv`)
