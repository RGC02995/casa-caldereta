...
### Fix — la sincronización iCal borraba todas las fechas de Booking.com en la segunda ejecución ✅ COMPLETADO (2026-07-22)
Usuario reportó que tras la primera sincronización correcta, una ejecución posterior del cron (cada 15 min, contra las URLs reales de Airbnb/Booking.com en producción) borraba por completo las fechas de Booking.com y dejaba de sincronizarse — Airbnb seguía funcionando. Grave: `BlockedPeriodModel` es la fuente que usa `hasBlockedConflict()` para evitar reservas solapadas entre plataformas, así que este bug podía permitir una doble reserva real.

- **Causa** (`backend/src/services/ical-sync.service.ts`, `syncPlatform()`): `deleteStaleExternal(origin, activeUids)` ejecuta `BlockedPeriodModel.deleteMany({ origin, externalUid: { $nin: activeUids } })` — si `activeUids` queda vacío, `$nin: []` no excluye nada y borra TODOS los bloqueos existentes de esa plataforma sin lanzar ningún error. `activeUids` queda vacío cuando el feed responde HTTP 200 con un `VCALENDAR` técnicamente válido (pasa el guard `hasValidCalendar` ya existente desde el 2026-07-05) pero sin ningún `VEVENT` dentro — indistinguible desde el código de un fallo transitorio del proveedor (rate limiting devolviendo un calendario vacío en vez de un error HTTP, corte puntual, etc.). Confirmado leyendo el código fuente real de `node-ical`: un HTTP no-2xx sí lanza excepción y ya queda protegido por el `try/catch` existente; el agujero era específicamente "200 OK, calendario válido, cero eventos dentro"
- **Investigación exhaustiva de las hipótesis planteadas por el usuario** (fusión vs. sobrescritura, caché, formato de fechas distinto entre plataformas, rate limiting, colisión de variables de entorno) — todas verificadas leyendo el código real, no por inferencia: colisión de variables descartada (`airbnbIcalUrl`/`bookingIcalUrl` son campos distintos sin solapamiento), caché descartada (el único `Cache-Control` del backend es en la respuesta que exponemos hacia fuera, no afecta a la lectura del feed de Booking), ningún otro proceso escribe/borra `BlockedPeriodModel` con origen booking (un solo cron registrado, sin duplicados)
- **"El problema de la variable"** que el usuario recordaba haber resuelto en la sesión anterior nunca llegó a commitearse — era el cambio `toUtcMidnight()` (normaliza fechas `VALUE=DATE` a medianoche UTC exacta porque `node-ical` las construye con la hora local del proceso, confirmado en su código fuente); fix real y correcto, ya cubierto por tests, pero sin relación con el borrado — commiteado junto con este fix por estar en los mismos ficheros
- **Fix**: en `syncPlatform()`, si `activeUids.length === 0` tras procesar el feed, se omite la llamada a `deleteStaleExternal()` y se avisa por `console.warn` — mismo patrón "ante la duda, no borrar" que el guard `hasValidCalendar` ya existente. Trade-off aceptado: si una plataforma tiene de verdad cero reservas activas, sus bloqueos ya cancelados no se limpian hasta la siguiente sync con ≥1 evento activo (se autocorrige solo); el efecto colateral en el peor caso es una fecha mostrada como no disponible cuando sí lo estaría, nunca una fecha liberada por error
- **2 tests nuevos** en `ical-sync.service.spec.ts`: reproduce el bug exacto (feed con eventos → feed vacío-pero-válido → el bloqueo debe seguir existiendo) + confirma que el guard es por-plataforma y no bloquea el borrado normal de la otra plataforma en la misma ejecución. El test `HALLAZGO` existente se verificó a mano tras el fix: sigue en verde, aunque el mecanismo interno cambia (antes pasaba parcialmente gracias a una instancia oculta de este mismo bug)
- 313/313 tests backend en verde, `tsc` sin errores
- **Bug secundario encontrado de paso, fuera de alcance por decisión del usuario** — ver "Pendientes" abajo (`upsertExternal()` sin filtrar por `origin`, mismo UID en Airbnb y Booking cambiaría de plataforma silenciosamente)
- **Sin commitear todavía**
